...
void runcmd(struct cmd* cmd) {
  ...
  switch (cmd->type) {
    ...
    case PIPE:
      ...
      // 这个系统调用创建了一个管道，p会返回管道两个端口的文件描述符，一读一写，p[0]是读口，p[1]是写端口，
      assert(syscall(SYS_pipe, p) >= 0);
      // 这里调用了一次fork，fork会把刚刚创建的文件描述符也给复制，
      // 相当于现在父子进程共享管道(管道不属于进程本身，只能通过端口访问)
      if (syscall(SYS_fork) == 0) {
        //对于第一个子进程，关闭stdout
        syscall(SYS_close, 1);
        //新分配一个空的文件描述符，把p[1]描述符复制一份给这个新描述符(因为刚刚关闭了描述符1，所以这个新分配的就是1)，此时标准输出就指向了管道的写口
        syscall(SYS_dup, p[1]);
        //在子进程中关闭管道两个端口
        syscall(SYS_close, p[0]);
        syscall(SYS_close, p[1]);
        //这个子进程递归调用runcmd函数，这个函数是不会返回的，执行到最底下的时候直接exit了
        runcmd(pcmd->left);
      }
      if (syscall(SYS_fork) == 0) {
        //对于第二个子进程，关闭标准输入
        syscall(SYS_close, 0);
        //把P[0]复制给标准输入(文件描述符0)，此时该进程的输入就会从管道中读取
        syscall(SYS_dup, p[0]);
        syscall(SYS_close, p[0]);
        syscall(SYS_close, p[1]);
        runcmd(pcmd->right);
