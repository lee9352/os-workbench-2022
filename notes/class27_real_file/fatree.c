...
void *cluster_to_sec(int n) {
  ...
  u32 DataSec = hdr->BPB_RsvdSecCnt + hdr->BPB_NumFATs * hdr->BPB_FATSz32;//第一个data sec起始位置
  DataSec += (n - 2) * hdr->BPB_SecPerClus;//根目录簇号为2
  return ((char *)hdr) + DataSec * hdr->BPB_BytsPerSec;//感觉这里最后得到的是字节位置,而不是扇区号
}
...
void dfs(u32 clusId, int depth, int is_dir) {
  ...
  for (; clusId < CLUS_INVALID; clusId = next_cluster(clusId)) {
    if (is_dir) {
      int ndents = hdr->BPB_BytsPerSec * hdr->BPB_SecPerClus / sizeof(struct fat32dent);//一个目录会占据一个簇，这里计算该目录下最多有多少项
